#!/usr/bin/env python3
# kaleido_stack_inked_offline.py — Four-layer composite visualizer, chunky-source +
# cartoon-ink aesthetic.  Derived from kaleido_stack_offline.py; the smooth version
# is untouched — both live side-by-side.
#
# Pipeline per frame:
#   Image layers 1/2/3 → render at 280p source → upscale to canvas →
#   3-pass contrast crush → composite on screen → strings (full canvas) →
#   Sobel edge ink overlay → pipe to ffmpeg → (abstrakt.sh folds via frei0r kaleido)
#
# The ink is applied BEFORE frei0r folding so each mandala wedge contains
# matching ink lines, reinforcing the 12-fold symmetry visually.
#
# Source resolution default 498×280 (280p).  Env-var configurable:
#   KSTACK_LOW_RES_W / KSTACK_LOW_RES_H  — source resolution per image layer
#   KSTACK_SHARPEN_PASSES / KSTACK_SHARPEN_FACTOR — contrast crush tuning
#   KSTACK_INK_THRESHOLD  — Sobel magnitude cutoff (higher → sparser ink)
#   KSTACK_INK_OPACITY    — alpha of ink lines (255 = fully opaque black)
#
# Expected: ~8-15 fps at 4K (image layers at 139k pixels, ink Sobel on full canvas).
...
# ── Chunky-source render budget ────────────────────────────────────────────────
# Image layers 1/2/3 render at this fixed low resolution, then upscale to canvas.
# Default 280p (139k pixels).  At 4K canvas the upscale is ~17× linear — the
# pixelation and contrast crush become the aesthetic, not a flaw.
LOW_RES_W = int(os.environ.get("KSTACK_LOW_RES_W", 498))
LOW_RES_H = int(os.environ.get("KSTACK_LOW_RES_H", 280))

# Contrast crush: 3 passes of (x-128)*factor+128.  Factor 2.0 compounds to ~8×
# effective contrast, pushing upscaling softness into hard pixel-art edges.
SHARPEN_PASSES = int(os.environ.get("KSTACK_SHARPEN_PASSES", 3))
SHARPEN_FACTOR = float(os.environ.get("KSTACK_SHARPEN_FACTOR", 2.0))

# Ink overlay: Sobel edge magnitude hard-threshold.  No partial alpha — either
# pure black ink or fully transparent.  This eliminates white-residue halos that
# ruin naive edge-overlay attempts.
...
CENTER_PULSE_DECAY = 0.7
CENTER_PULSE_RATE  = 0.3
...
print(
    f"[kaleido_stack_inked] {WIDTH}x{HEIGHT} @ {FPS}fps  "
    f"source={LOW_RES_W}x{LOW_RES_H}  "
    f"spk={SPK_COLS}×{SPK_ROWS}={NUM_SPEAKERS}  strings={N_STRINGS}  "
    f"ink_thr={INK_THRESHOLD}",
    flush=True,
)
...
def _sharpen_pass(surf: pygame.Surface, passes: int = SHARPEN_PASSES,
    ...
    pixel = (pixel - 128) * factor + 128, clipped. Returns surf for chaining."""
    arr = pygame.surfarray.pixels3d(surf)
...
def _ink_overlay(composite_surf: pygame.Surface,
                 threshold: int = INK_THRESHOLD,
                 opacity: int   = INK_OPACITY) -> pygame.Surface:
    """GIMP-style cartoon ink layer via Sobel edge detection.

    Hard threshold only (no partial alpha) — eliminates white-residue halos that
    appear with soft-edged approaches.  Returns an SRCALPHA surface: edge pixels
    are pure black at `opacity`; everything else is fully transparent.

    Applied to the composite before frei0r kaleido fold so ink lines become part
    of each mandala wedge and reinforce the 12-fold rotational structure."""
    arr  = pygame.surfarray.array3d(composite_surf).astype(np.float32)
    luma = arr[:, :, 0] * 0.299 + arr[:, :, 1] * 0.587 + arr[:, :, 2] * 0.114

    gx  = _sobel(luma, axis=0)
    gy  = _sobel(luma, axis=1)
    mag = np.sqrt(gx * gx + gy * gy)

    is_edge = mag > threshold

    w, h    = composite_surf.get_size()
    overlay = pygame.Surface((w, h), pygame.SRCALPHA)
    overlay.fill((0, 0, 0, 0))

    alpha_arr = pygame.surfarray.pixels_alpha(overlay)
    alpha_arr[is_edge] = opacity
    del alpha_arr

    return overlay


# ── String physics helpers ─────────────────────────────────────────────────────
...
if _ch not in (1, 2):
    sys.exit(f"[kaleido_stack_inked] unsupported channel count: {_ch}")
...
lr_surf     = pygame.Surface((LOW_RES_W, LOW_RES_H))
spk_surf    = pygame.Surface((LOW_RES_W, LOW_RES_H))
_GHOST_SURF = pygame.Surface((WIDTH, HEIGHT))
_GHOST_SURF.set_colorkey((0, 0, 0))

seed    = audio_hash_seed(AUDIO_FILE)
print(f"[kaleido_stack_inked] generating mandala (seed={seed & 0xFFFF:04x}…)", flush=True)
mandala = generate_mandala(LOW_RES_W, LOW_RES_H, seed)
print(f"[kaleido_stack_inked] mandala ready", flush=True)
...
while True:
    ...
    # ── Layers 1 & 2: warped mandala (280p → upscale → contrast crush) ────────
    ...
    if center_pulse > 1.005:
        ...
        ox = (WIDTH  - new_w) // 2
        oy = (HEIGHT - new_h) // 2
        screen.blit(layer3_pulsed, (ox, oy))
    else:
    ...
    # composite into 12 wedges, making ink a structural part of the mandala.
    ink = _ink_overlay(screen)
    screen.blit(ink, (0, 0))
    ...
    if window_frames >= FPS or frame_idx == 1:
        ...
        print(
            f"[kaleido_stack_inked] frame {frame_idx}"
            f"  fps={fps_r:.1f}"
...
rc = _proc.wait()
if rc != 0:
    sys.exit(f"[kaleido_stack_inked] ERROR: ffmpeg exited with code {rc}")

elapsed    = time.time() - render_start
e_min, e_s = divmod(int(elapsed), 60)
print(f"[kaleido_stack_inked] Done in {e_min}:{e_s:02d}. Output: {OUT_FILE}", flush=True)
